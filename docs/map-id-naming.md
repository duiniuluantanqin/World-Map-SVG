...
| 14 | VN | 64 个 id：VN 64 | VN 9 条离岛/三角洲碎块；NE 3 处地名误写改用正确英文名 |
| 15 | ID | 186 个 id：ID 186 | ID `path26597`（Papua+WestPapua 冗余聚合轮廓）、`path7234`（近 Atauro 歧义岛） |
...
| 20 | GB | 79 个 id：GB 79 | GB `path12075`（福斯湾岸碎块，爱丁堡/法夫歧义） |
| 21 | JP, IN | 63 个 id：JP 43、IN 20 | 无 |
| 22 | PG, SB | 75 个 id：PG 51、SB 24 | 无（离岛已按主体省采纳） |
...
批次 16 说明：格陵兰（GL）为字母码国家，NE 10m 用 2018 年前旧制：`GL-KU`（Kujalleq 南）、`GL-QE`（Qeqqata 西中）、`GL-SM`（Sermersooq 东南）、`GL-QA`（Qaasuitsup 北/西北，2018 年已拆为 Avannaata+Qeqertalik）＋`GL-UO`（Pituffik，图勒空军基地 1007 km²）。本图把格陵兰海岸线拆成 139 个叶 path 碎块（峡湾/离岛极多），NE 海岸线与本图不重合，故命名采用「质心落点 → IoU → 顶点投票 → 最近质心（排除 `GL-UO` 微区）」四级兜底，139 条全部定名（`GL-QA` 83、`GL-SM` 25、`GL-KU` 20、`GL-QE` 11）。`GL-UO`（Pituffik）无对应碎块（图勒基地在图上的碎块并入 Qaanaaq 一带，归 `GL-QA`）。西岸约 68°N、64.5°N 的市界附近个别碎块按最近质心近似归属，可能存在 ±0.5° 的边界误差，但格陵兰市级为低密度/低价值区，可接受。
...
批次 20 说明：英国（GB）为县级字母码（ISO 3166-2:GB 三级 county/unitary 码，`GB-XXX`）。NE 10m 有 232 个县级 feature，本图只绘 ~102 个（80 魔数 + 22 原有已命名），本批 79 条命名（`GB-ABD` 阿伯丁郡、`GB-AGB` 阿盖尔-比特、`GB-CHW` 柴郡西等），离岛按面积加后缀（如 `GB-ABD-1/2`、`GB-AGB-1`）。原有 22 条命名多半为与原图一致的 3 字母大写（`GB-AGY`/`GB-CAM`），少数为小写英文（`GB-antrim`/`GB-dyfed`/`GB-gwynedd`/`GB-london`/`GB-south-glamorgan`），本批新增统一用 NE 大写码、原有小写不追溯（记入已知问题 #2 同类）。`path12075`（357 km²，56.00N/3.33W，福斯湾沿岸，近爱丁堡/法夫交界）与 NE 郡面 IoU 低、归属歧义，保持魔数。

批次 21 说明：日本（JP）数字码→英文名，印度（IN）字母码。日本 47 都道府县，本批 43 条魔数命名，7 条已有（`JP-hokkaido`/`JP-tokyo`/`JP-osaka`/`JP-kyoto`/`JP-nagasaki`/`JP-nara`/`JP-hiroshima`）不动；NE 的 `name_en` 带「Prefecture」后缀（如 Yamaguchi Prefecture），生成的 id 会多出 `-prefecture`，本批在命名后统一剥离该后缀以对齐原图的英文名习惯（`JP-yamaguchi`/`JP-niigata-1`），山口/新潟 2 县含离岛按面积加后缀。印度本图只绘 20 个邦（余并入或未绘制），字母码（`IN-MH` 马哈拉施特拉、`IN-KA` 卡纳塔克等）。

批次 22 说明：巴布亚新几内亚（PG）、所罗门群岛（SB）字母码群岛国。NE 的海岛面与本图离岛质心不重合，12 条离岛手工按地理归属采纳：PG 的 `path14150`（巴布亚湾）→`PG-GPK`、`path14202`/`path14204`（布干维尔）→`PG-NSB`；SB 的 `path4726/4728/4732/4734/4736/4738/4744`（新乔治亚群岛：韦拉拉韦拉/科隆邦加拉/新乔治亚/伦多瓦/万古努等）→`SB-WE`（西部省，NE 该省面未覆盖图上碎块）。马当省（PG-MBA）在本图含 12 块离岛（俾斯麦海诸岛）。

批次 23 说明：柬埔寨（KH）数字码→英文名（`KH-phnom-penh`、`KH-siem-reap`），希腊（GR）字母码（`GR-A`…`GR-M` 13 大区），捷克（CZ）字母码，巴哈马（BS）字母码地区。BS 为群岛，多数地区＝单个岛可直接落点；两条大岛（`path6094`＝阿巴科岛 1662 km²、`path6096`＝伊柳塞拉岛 723 km²）各自横跨多个 NE 地区（北/中/南阿巴科、北/中/南伊柳塞拉），归属无法唯一确定 → 保持魔数。**阿塞拜疆（AZ）、斯洛文尼亚（SI）、斯里兰卡（LK）整国跳过**：NE 10m 给的是区县/市镇级（AZ 78 个 rayon、SI 193 个 občina、LK 25 个 district），而本图绘的是更粗的经济区/统计区/省，粒度错配，强行落点会把一个区误冠以某个下辖县名，需另寻省级数据后再命名（记入已知问题 #2 同类）。

批次 24 说明：瓦努阿图（VU）6 省（字母码，本图各岛按面积归属），几内亚比绍（GW）9 区，摩洛哥（MA）数字码旧 16 区（本图 12 单元，含西撒哈拉沿岸 MA-14/15/16 争议区按图归属），博茨瓦纳（BW）9 区。GW `path3432`（87 km²，11.90N/16.28W 近岸小岛）按卡谢乌区 `GW-CA` 采纳。**布基纳法索（BF）、几内亚（GN）整国跳过**：NE 10m 给的是省级（BF 45 province）/县级（GN 34 préfecture），本图绘的是更粗的大区（BF 13 大区 / GN 8 大区+科纳克里），粒度错配，同 AZ/SI/LK。

批次 25 说明：斐济（FJ）4 大区+罗图马（5 个字母码，本图按岛拆分），安道尔（AD）7 教区（数字码→英文名）。**北马其顿（MK，NE 84 市镇 vs 本图 8 统计区）、佛得角（CV，NE 22 市镇 vs 本图 8 岛）、尼泊尔（NP，NE 14 旧 Zone vs 本图省）、拉脱维亚（LV，NE 119 市镇 vs 本图 5 规划区）、不丹（BT，NE 20 宗 vs 本图 4 区）、南极洲（AQ，无行政区）整国跳过**——同为 NE 粒度与本图不匹配或无行政区可依，保持魔数。
...
| world-states-provinces.svg | `VN` 9 条：`path81220`（690 km²，10.29N/103.99E，富国岛）、`path5498`（281，21.19N/107.46E）、`path5496`（178，20.81N/107.01E）、`path5500`（112，20.98N/107.50E）、`path5494`（56，20.91N/106.81E）、`path5502`（47，21.41N/107.92E），及 `path3476`（272，10.55N/106.80E）、`path3474`（185，9.65N/106.17E）、`path3480`（58，12.62N/109.38E） | 富国岛（属 Kiên Giang）、下龙湾/海防离岛（属 Quảng Ninh/Hải Phòng）、湄公河口碎块、芽庄城市块。前 6 条为离岛；`path3474` 被 propose 误配 Bạc Liêu（d74km），`path3480` 被误配 Khánh Hòa（实为芽庄城址）均已回退 | 保持魔数 |
| world-states-provinces.svg | `ID` `path26597` | 398812 km²，bbox 横跨 Papua+WestPapua 两省（对 `ID-PA` IoU 0.87、对 `ID-PB` IoU 0.84），是两省合并的冗余聚合轮廓；其下 `ID-PA`（`path19049`，305864 km²）与 `ID-PB`（`path7063`，92340 km²）已独立绘制并命名 | 保持魔数 |
| world-states-provinces.svg | `ID` `path7234` | 233 km²（-8.20N/125.60E），近 Atauro/Alor（东帝汶与 NTT 交界），NE 无对应、NTT/Maluku 归属歧义 | 保持魔数 |
